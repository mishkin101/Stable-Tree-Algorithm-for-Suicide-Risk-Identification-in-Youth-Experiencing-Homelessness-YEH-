...
Thus:  
$$
(\mathbf{u}^p,\mathbf{l}^p,\bm{C}^p,k^p) \in \mathbb{R}^{|\mathcal{N}|} \times \mathbb{R}^{|\mathcal{N}|} \times \{0,1\}^{|\mathcal{C}| \times \max_j c_j} \times [K]
$$

```{=html}
<aside class="notes">
Explain that numeric bounds and category masks fully define the leaf region.
</aside>
```    

---

## Formal Path Representation

\[
(p):
\bigl(\mathbf{u}^p,\mathbf{l}^p,\bm{C}^p, k^p\bigr)
\in
\mathbb{R}^{|\mathcal{N}|}\times
\mathbb{R}^{|\mathcal{N}|}\times
\{0,1\}^{|\mathcal{C}| \times \max_j c_j}\times
[K]
\]

- $\mathbf{u}^p_j, \mathbf{l}^p_j$ define numeric intervals.
- $\bm{C}^p[j, :]$ indicates allowed categories for feature $j$.
- $k^p$ is the predicted class for inputs in this region.

---

## Tree Representation & Non-Uniqueness

- The tree $\mathbb{T}$ is the collection of its $T$ paths:

$$
\mathcal{P}(\mathbb{T}) = \{p_1, \dots, p_T\}
$$
